...
function create(program: commander.Command) {
  program
    ...
    .option('-c, --cmd <string>', 'Command to execute after starting the terminal')
    .option('-t, --timeout <number>', 'Timeout after terminal should be closed in seconds')
    .option(...portOption)
    .option(...baudRateOption)
    .action(async (options: Options) => {
      const baudRate = options.baudRate !== undefined ? parseInt(options.baudRate, 10) : defaultBaudRate
      const timeout = options.timeout !== undefined ? parseInt(options.timeout, 10) : undefined
      startTerminal({
        baudRate,
