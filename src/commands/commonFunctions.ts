...
export const mergeEnvironments = (settingsFile: SettingsFile, selectedEnvironments: string[]): SettingsFile => {
  ...
  const settingsSetMerged: SettingsSet = environmentKeys.reduce((mergedSet, envKey) => {
    const env = settingsFile[envKey] as SettingsSet
    return {
      ...mergedSet,
      ...env
...
export const checkManifestCompatibility = (settingsFile: SettingsFile) => {
  ...
  if (version < minCliVersion) {
    console.warn(`CLI version ${version} do not support manifest version ${manifestVersion}. Upgrade nodemcu-app to at least ${minCliVersion}`)
  }
}